...
public class PrintGalaxyTest
{
	@Disabled
	@Test
	void PrintGalaxyTest(){
		...
		try
		{
			String expectedPath = "out/test/Output/OPG11.txt";
			String actualPath = "out/Output/OPG11.txt";
			
			String expected = "Blue Player (The Emirates Of Hacan)\n" +
					"\tMecatol Rex\n" +
					"Red Player (The Federation Of Sol)\n" +
					"\tVega Major\n" +
					"\tVega Minor";
			galaxy.PrintGalaxy(galaxy);
			Files.write(Paths.get(expectedPath),expected.getBytes());
			assertEquals(Files.readAllLines(Paths.get(expectedPath)), Files.readAllLines(Paths.get(actualPath)));
		}
		catch (Exception e){
			e.getMessage();
		}
